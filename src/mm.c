...
void* mm_malloc (size_t size) {
  ...
  { 
    BlockInfo *splitFreeBlock;
    splitFreeBlock = (BlockInfo *)UNSCALED_POINTER_ADD(ptrFreeBlock,reqSize);
    removeFreeBlock(ptrFreeBlock);
    ptrFreeBlock->sizeAndTags = reqSize|(precedingBlockUseTag)|0x5;

    splitFreeBlock->sizeAndTags = (blockSize-reqSize)|0x2;
    ((BlockInfo *)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize-WORD_SIZE))->sizeAndTags=(blockSize-reqSize)|0x2;
    insertFreeBlock(splitFreeBlock);
  }
  else {
    ptrFreeBlock->sizeAndTags |= 0x5;
    ((BlockInfo *)UNSCALED_POINTER_ADD(ptrFreeBlock,blockSize))->sizeAndTags |=0x2;
    removeFreeBlock(ptrFreeBlock);
  }
...
void mm_free (void *ptr) {
  ...
  blockInfo = (BlockInfo *)UNSCALED_POINTER_SUB(ptr, WORD_SIZE); // ptrFreeBlock
   // printf("real freeing %p\n",blockInfo );
  payloadSize = SIZE(blockInfo->sizeAndTags) - WORD_SIZE;
  ptrBoundaryTag = (void *)UNSCALED_POINTER_ADD(blockInfo,payloadSize);
  // printf("boundarytag is %ld\n", *(size_t *)ptrBoundaryTag);
  followingBlock = (BlockInfo *)UNSCALED_POINTER_ADD(ptr, payloadSize);
  // printf("the followingBlock addr is %p\n", followingBlock);
 // *ptrBoundaryTag = blockInfo->sizeAndTags & (~0x5);
 // printf("4\n");
  *(size_t *)(blockInfo) = *(size_t *)ptrBoundaryTag = (*(size_t *)(blockInfo))&(~0x5);
 // blockInfo->sizeAndTags &= (~0x5);
  //blockInfo->sizeAndTags = *ptrBoundaryTag ;
  // printf("3\n");
  (followingBlock->sizeAndTags) &= (~0x2);
   // printf("2\n");
...
void* mm_realloc(void* ptr, size_t size) {
  ...
  size_t blockUseTag;
  size_t reqSize;
  size_t currentTotalSize, currentPayloadSize;
  size_t adjustedSize;
  size_t copySize;
  BlockInfo * oldptr;
  BlockInfo * newptr;
  ...
  if (size <= (MIN_BLOCK_SIZE-WORD_SIZE)) {
    // Make sure we allocate enough space for a blockInfo in case we
    // free this block (when we free this block, we'll need to use the
    // next pointer, the prev pointer, and the boundary tag).
    reqSize = MIN_BLOCK_SIZE;
  ...
  currentTotalSize = SIZE(oldptr->sizeAndTags);
  currentPayloadSize = currentTotalSize - WORD_SIZE;

  if (currentTotalSize>=reqSize)
  {
    if (currentPayloadSize>=(size+WORD_SIZE))
    {
      oldptr->sizeAndTags &= (~0x4);
      ((BlockInfo *)UNSCALED_POINTER_ADD(oldptr,currentPayloadSize))->sizeAndTags = size;
      return ptr;
    }
  }

  adjustedSize = size * 2 + WORD_SIZE ;

  ptrNew = mm_malloc(adjustedSize);
  newptr = (BlockInfo *)UNSCALED_POINTER_SUB(ptrNew,WORD_SIZE);
  newptr->sizeAndTags &= (~0x4);
  reqSize = SIZE(newptr->sizeAndTags);
  *(size_t *)UNSCALED_POINTER_ADD(newptr,reqSize - WORD_SIZE)=size;
  
  if ((oldptr->sizeAndTags & 0x4)==0)
  {
    copySize = *(size_t *)UNSCALED_POINTER_ADD(ptr,currentPayloadSize-WORD_SIZE);
  }else{
    copySize = currentPayloadSize;
  }
